...
payors = ['BlueShield', 'Medicare', 'HealthFirst', 'WellCare', 'Aetna', 'Cigna', 'UnitedHealthcare', 'Anthem', 'Humana']
def assign_payor_v2():
    prob = random.random()
    if prob < 0.4:  # 40% probability
        ...
        remaining_payors = [payor for payor in payors if payor not in ['Medicare', 'Aetna', 'BlueShield']]
...
for _ in range(9230):
    ...
    patients.append([patient_id, first_name, last_name, age, address, state, 
                     payor])

# Create a DataFrame from the patient data
patient_df = pd.DataFrame(patients, columns=['patient_id', 'first_name', 'last_name', 'age', 'address', 'state', 'payor'])
...
START_TREND = datetime.date.today() - datetime.timedelta(days=730)  # Approximate two years in days
END_TREND = datetime.date.today() - datetime.timedelta(days=365)
PATIENT_VOLUME_INITIAL = 15
PATIENT_VOLUME_FINAL = 120
...
def get_claim_status(how_old_fraction):
    REJECTION_RATE = .055
    if(age > 30):
        paid_p = .95 * pow(how_old_fraction, .1)
    ...
    return random.choices(['Paid', 'Unpaid', 'Rejected'], [paid_p, unpaid_p, reject_p])[0]

while date_generated <= end_date:
    age = (end_date - date_generated).days
    how_old_fraction = age / HISTORY_IN_DAYS

    num_patients_sampled=round(get_patient_volume_for_day(date_generated)) + random.randint(0,17)
    for idx, patient in patient_df.sample(n=num_patients_sampled).iterrows():
        ...
        claims.append({
            ...
            'SubmissionDate': date_generated - datetime.timedelta(days=random.randint(1, 5)),
            'PaymentDate': date_generated + datetime.timedelta(days=random.randint(1, 5)),
            'PayorName': payor,
